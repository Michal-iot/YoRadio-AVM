...
#include "../displays/fonts/Decoderr11pt7b_mono.h"  // przełączanie dużego/malego zestawu fontów dla zegara
...
//  _date->setGfxFont(&Clock_GFXfont); //test
...
//  _refreshWeatherUI();
  ...
  // --- Dodane reset zegara przy starcie ---
  if (_clock) _clock->reset();
}
...
void Display::_swichMode(displayMode_e newmode) {
  ...
  // ── PLAYER ──────────────────────────────────────────────────────────────
  if (newmode == PLAYER) {
    clockfont_set_large(false);                        // 1. font mały

    // 2. przywróć pozycje zegara i daty z konfiguracji (nie moveBack — bo wygaszacz je zmienił)
    if (_clock) {
      auto dc_clock = getclockConf();
      _clock->moveTo({dc_clock.left, dc_clock.top, 0});
    }
    if (_date) {
      auto dc = getDateConf();
    ...
    if (_clock) _clock->reset();                       // 3. reset w dobrej pozycji z małym fontem
    ...
    // 4. data — update po przywróceniu pozycji
    if (_date) _date->update();
  }

  // ── SCREENSAVER / SCREENBLANK ────────────────────────────────────────────
  if (newmode == SCREENSAVER || newmode == SCREENBLANK) {
    if (_clock) {
      clockfont_set_large(true);
      _clock->reset();
    }
    ...
    // Tylko tryby które NIE są PLAYER i NIE są SCREENSAVER resetują timer
    // isScreensaver zostaje bez zmian - nie dotykamy jej tutaj!
  ...
  if (newmode == SLEEPING)  { _showDialog("SLEEPING"); delay(2000); dsp.clearDsp(); config.doSleepW(); }
...
#endif
  static int lastSSMove = -1;
  if (!config.isScreensaver) {
    lastSSMove = -1;
  }

  if (config.isScreensaver) {
    int sec30 = (network.timeinfo.tm_min * 2) + (network.timeinfo.tm_sec >= 30 ? 1 : 0);

    if (sec30 != lastSSMove) {
      lastSSMove = sec30;

      WidgetConfig ccfg = getclockConf();
      uint16_t realClockH = _clock->clockHeight();
      uint16_t contentH   = (TIME_SIZE < 19)
        ? (uint16_t)(TIME_SIZE * CHARHEIGHT)
        : (uint16_t)_clock->dateSize();

      // Y: pełny zakres ekranu — baseline musi być >= realClockH od góry
      int16_t ymin = (int16_t)(TFT_FRAMEWDT + realClockH);
      int16_t ymax = (int16_t)dsp.height() - (int16_t)contentH - (int16_t)(TFT_FRAMEWDT * 2);
      // USUNIĘTO blok SAVER_Y_MIN który obcinał ymax do ~4px

      if (ymax < ymin) ymax = ymin;
      uint16_t ft = (uint16_t)random(ymin, ymax + 1);

      // X: pełny zakres ekranu
      uint16_t cw   = _clock->clockWidth();
      int16_t  xmin = (int16_t)TFT_FRAMEWDT;
      int16_t  xmax = (int16_t)dsp.width() - (int16_t)cw - (int16_t)TFT_FRAMEWDT;
      if (xmax < xmin) xmax = xmin;
      uint16_t lt = (uint16_t)random(xmin, xmax + 1);

      MoveConfig pos;
      pos.y = ft;
      pos.x = (ccfg.align == WA_CENTER)
        ? (int16_t)(lt - ((int16_t)dsp.width() - (int16_t)cw) / 2)
        : (int16_t)lt;
      pos.width = 0;

      Serial.printf("[SAVER] ymin=%d ymax=%d ft=%d xmin=%d xmax=%d lt=%d clockH=%d\n",
                    ymin, ymax, ft, xmin, xmax, lt, realClockH);
      ...
      if (_date) {
        ...
        MoveConfig dm;
        dm.width  = dc.width;
        dm.y = (uint16_t)((int16_t)pos.y + ((int16_t)dc.widget.top - (int16_t)cc.top));
        dm.x = (uint16_t)((int16_t)dc.widget.left + ((int16_t)pos.x - (int16_t)cc.left));
        _date->moveTo(dm);
      }
    }
  }
...
void Display::_rebuildUI() {
  // --- Ustawienie fontu w zależności od trybu przed budową ---
  if (_mode == SCREENSAVER || _mode == SCREENBLANK) {
    clockfont_set_large(true);
  } else {
    clockfont_set_large(false);
  }
  ...
  // --- Po zbudowaniu widgetów reset zegara ---
  if (_clock) _clock->reset();
...
void Display::_endRebuild() {
  _locked = false;
  // --- Reset zegara po zakończeniu przebudowy ---
  if (_clock) _clock->reset();
}
