...
if (request->hasParam("lsEnabled")) {
    touched = true;
    int v = request->getParam("lsEnabled")->value().toInt() ? 1 : 0;
    if ((uint8_t)v != config.store.lsEnabled) {
        config.store.lsEnabled = (uint8_t)v;
        changed = true;
        Serial2.println(v ? "LED:ON" : "LED:OFF");
    }
}
  ...
  // --- LED strip: model/effect ---
   if (request->hasParam("lsModel")) {
    touched = true;
    int v = constrain(request->getParam("lsModel")->value().toInt(), 0, 10);
    if ((uint8_t)v != config.store.lsModel) {
      config.store.lsModel = (uint8_t)v;
      changed = true;
      // Wyślij komendę do ESP (numer efektu)
      Serial2.printf("EFF:%d\n", v);
    }
  }
  ...
  if (request->hasParam("lsBrightness")) {
    ...
    if ((uint8_t)v != config.store.lsBrightness) {
      ...
      changed = true;
      // WYŚLIJ KOMENDĘ DO ESP (przelicz 0-100 na 0-255)
      Serial2.printf("BRI:%d\n", (int)map(v, 0, 100, 0, 255));
    }
  }
