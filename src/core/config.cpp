...
#endif
#if IR_PIN != 255
    // Dodaj IR_PIN do maski jezeli jest pinem RTC
    if (rtc_gpio_is_valid_gpio((gpio_num_t)IR_PIN)) {
        rtc_gpio_pullup_en((gpio_num_t)IR_PIN);
        rtc_gpio_pulldown_dis((gpio_num_t)IR_PIN);
        mask |= (1ULL << IR_PIN);
    }
#endif
    if (mask != 0) { esp_sleep_enable_ext1_wakeup(mask, ESP_EXT1_WAKEUP_ANY_LOW); }
    if (config.sleepfor > 0) { esp_sleep_enable_timer_wakeup(config.sleepfor * 60ULL * 1000000ULL); }
    esp_deep_sleep_start();
}

void Config::doSleepW() {
    Serial.println("doSleepW start");
    Serial.flush();
    display.deepsleep();
...
#endif
    if (mask != 0) { esp_sleep_enable_ext1_wakeup(mask, ESP_EXT1_WAKEUP_ANY_LOW); }
        Serial.println("before deep_sleep_start");
    Serial.flush();
    delay(100);
    esp_deep_sleep_start();
    esp_deep_sleep_start();
