...
/* Sprawdza co minutę czy znana sieć WiFi jest dostępna - używane w trybie AP */
void checkKnownNetworks(void* pvParameters) {
  while (true) {
    delay(60000); // czekaj 1 minutę
    Serial.println("##AP## Scanning for known networks...");
    int n = WiFi.scanNetworks();
    bool found = false;
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < config.ssidsCount; j++) {
        if (strcmp(WiFi.SSID(i).c_str(), config.ssids[j].ssid) == 0) {
          Serial.printf("##AP## Found known network: %s, restarting...\n", config.ssids[j].ssid);
          found = true;
          break;
        }
      }
    ...
    if (found) {
      if (WiFi.softAPgetStationNum() == 0) {
        Serial.println("##AP## No clients connected, restarting...");
        delay(500);
        ESP.restart();
      } else {
        Serial.println("##AP## Client connected to AP, skipping restart.");
      }
    } else {
      Serial.println("##AP## No known networks found.");
    }
  }
  vTaskDelete(NULL);
}
...
void MyNetwork::requestTimeSync(bool withTelnetOutput, uint8_t clientId) {
  if (withTelnetOutput) {
    ...
    //  Serial2.printf("##SYS.DATE#: %s%03d:%02d\r\n", timeStringBuff, config.store.tzHour, config.store.tzMin);
    } else {
...
void MyNetwork::raiseSoftAP() {
  ...
  // Sprawdzaj co minutę czy znana sieć jest dostępna
  xTaskCreatePinnedToCore(checkKnownNetworks, "checkKnownNetworks", 1024 * 4, NULL, 1, NULL, 0);
}
