...
#include "plugins/ledstrip/ledstrip.h"
#include "Arduino.h"
#include "options.h"
#include "config.h"
#include "controls.h"
...
// ===== Station select via long-press on encoder button =====
bool     stationSelectActive  = false;   // jesteśmy w trybie wyboru stacji
uint32_t stationSelectTimeout = 0;       // kiedy auto-zatwierdzić (millis)
#define  STATION_SELECT_TIMEOUT_MS 1500  // 3 sekundy na auto-zatwierdzenie

// unsigned long irPowerTimer = 0; // Długie trzymanie przycisku power
// bool powerButtonPressed = false;
// #define LONG_PRESS_TIME 1000 // Czas trzymania: 2 sekundy
...
void loopControls() {
    ...
    // ===== Auto-zatwierdzenie stacji po 3 sek braku aktywności =====
    if (stationSelectActive && millis() >= stationSelectTimeout) {
        stationSelectActive = false;
...
#        if defined(ENCODERS_INDEPENDENT)
    ...
    // ==========================================================
    if (first) {
        // ===== Tryb wyboru stacji via długie naciśnięcie =====
        if (stationSelectActive) {
            stationSelectTimeout = millis() + STATION_SELECT_TIMEOUT_MS;
            if (display.mode() != STATIONS) {
        ...
        // ----- Encoder 1 → Volume (normalny tryb) -----
...
#        else
    ...
    if (first) {
        // ===== Tryb wyboru stacji via długie naciśnięcie =====
        if (stationSelectActive) {
            stationSelectTimeout = millis() + STATION_SELECT_TIMEOUT_MS; // reset timeouta
            if (display.mode() != STATIONS) {
                display.putRequest(NEWMODE, STATIONS);
                return;
            }
...
void irWakeup() {
    ...
    // ESP32-S3: GPIO7 nie jest RTC, wiec wake cause to GPIO zamiast EXT1
    bool irWake = (wake == ESP_SLEEP_WAKEUP_EXT1 && (esp_sleep_get_ext1_wakeup_status() & (1ULL << IR_PIN)))
               || (wake == ESP_SLEEP_WAKEUP_GPIO);
    if (irWake) {
        Serial.println("IR wake → waiting for POWER code");
        if (rtc_gpio_is_valid_gpio((gpio_num_t)IR_PIN)) {
            rtc_gpio_deinit((gpio_num_t)IR_PIN); // RTC to GPIO (tylko klasyczny ESP32)
        }
        pinMode(IR_PIN, INPUT);
        irrecv.enableIRIn();
        bool          valid = false;
...
void irLoop() {
    if (irrecv.decode(&irResults)) {
        ...
        if (irResults.repeat) {
            ...
            irrecv.resume();
            return; // przy repeat nie sprawdzaj dalej kodów
        } else {
            irVolRepeat = 0;
        }
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (config.ircodes.irVals[target][j] == irResults.value) {
                    ...
                    switch (target) {
                        ...
                        //         // To jest moment pierwszego wciśnięcia
                        //         irPowerTimer = millis();
                        //         powerButtonPressed = true;
                        //         Serial.println("Przycisk POWER wciśnięty...");
                        //     } else {
                        //         // Przycisk jest trzymany - sprawdzamy czas
                        //         if (millis() - irPowerTimer > LONG_PRESS_TIME) {
                        //             Serial.println("DŁUGIE PRZYTRZYMANIE -> Deep Sleep");
                                    
                        //             ledstripOff(); // Twoja funkcja wyłączania LED
                        //             display.putRequest(NEWMODE, SLEEPING);
                                    
                        //             powerButtonPressed = false; // Reset, żeby nie zapętlić
                        //             delay(500); 
                        //         }
                        //     }
                        //     break;
...
void onBtnLongPressStart(int id) {
    switch ((controlEvt_e)id) {
        ...
        case EVT_ENCBTNB: {
            ...
            break;
#endif
            // Długie naciśnięcie enkodera → tryb wyboru stacji (oba tryby)
            stationSelectActive  = true;
            stationSelectTimeout = millis() + STATION_SELECT_TIMEOUT_MS;
            if (display.mode() != STATIONS) {
                display.putRequest(NEWMODE, STATIONS);
            }
...
void onBtnClick(int id) {
    ...
    switch (btnid) {
        ...
        case EVT_ENC2BTNB: {
            ...
            // ===== Zatwierdzenie stacji (tryb wyboru przez długie naciśnięcie) =====
            if (stationSelectActive && display.mode() == STATIONS) {
                stationSelectActive = false;
                display.putRequest(NEWMODE, PLAYER);
                if (config.lastStation() != display.currentPlItem) {
                    display.putRequest(CLOSEPLAYLIST, display.currentPlItem);
