...
static void formatDateCustom(char* out, size_t outlen, const tm& t, uint8_t fmt) {
  const char* month = (const char*)pgm_read_ptr(&(LANG::mnths[t.tm_mon]));
  const char* dow   = (const char*)pgm_read_ptr(&(LANG::dow[t.tm_wday]));
  ...
  switch (fmt) {
    ...
    switch (fmt) {
      ...
      default: // 16.09.2025 
        snprintf(out, outlen, "%s - %s. %s. %d", dow, d2, m2, y); //nprintf(out, outlen, "%s.%s.%d", d2, m2, y);
        break;
    }
  #endif
...
/* -------------------------------------------------------------------------
   Nowe funkcje pomocnicze korzystające z clockfont_api.h
   (zastępują stare wersje _charWidth, _textHeight, _textWidth)
   ------------------------------------------------------------------------- */
static inline const GFXglyph* glyphPtr(const GFXfont* f, unsigned char c) {
  if (!f) return nullptr;
  if (c < f->first || c > f->last) return nullptr;
  return f->glyph + (c - f->first);
}

static uint8_t _charWidth(unsigned char c) {
  const GFXfont* f = clockfont_get(config.store.clockFontId);
  if (!f) return TIME_SIZE;
  const GFXglyph* g = glyphPtr(f, c);
  return g ? pgm_read_byte(&g->xAdvance) : TIME_SIZE;
}

static uint16_t _textHeight() {
  const GFXfont* f = clockfont_get(config.store.clockFontId);
  if (!f) return TIME_SIZE;
  const GFXglyph* g = glyphPtr(f, '8');
  return g ? pgm_read_byte(&g->height) : TIME_SIZE;
}

static uint16_t _textWidth(const char* txt) {
  uint16_t w = 0;
...
void NumWidget::setText(const char* txt) {
  ...
  // Aktualizuj _textheight dla aktualnego fonta PRZED czyszczeniem
  uint16_t newH = _textHeight();
  if (newH != _textheight) _textheight = newH;
  ...
  if(clockfont_get(config.store.clockFontId)==nullptr) realth = _textheight * 8;
#endif

  if (_active) {
    // Czyść obszar max(stary, nowy) tekst, z pełną wysokością fonta + 1px margines od góry
    uint16_t clearLeft = (_oldleft == 0) ? _realLeft() : min(_oldleft, _realLeft());
    uint16_t clearW    = max(_oldtextwidth, _textwidth);
  #ifndef CLOCKFONT5x7
    int16_t  clearTop  = (int16_t)_config.top - (int16_t)realth;  // bez +1, pełna wysokość
    if (clearTop < 0) clearTop = 0;
    dsp.fillRect(clearLeft, (uint16_t)clearTop, clearW, realth + 1, _bgcolor);  // +1 zapas na dole
  #else
    dsp.fillRect(clearLeft, _config.top, clearW, realth, _bgcolor);
  #endif
  }
...
void NumWidget::_draw() {
  ...
  const GFXfont* f = clockfont_get(config.store.clockFontId);
  // Aktualizuj _textheight dla aktualnego fonta
  uint16_t newH = _textHeight();
  if (newH != _textheight) _textheight = newH;
  dsp.setTextSize(f ? 1 : TIME_SIZE);
  dsp.setFont(f);
  dsp.setTextColor(_fgcolor, _bgcolor);
#endif
  if(!_active) return;
...
//  Serial.printf("Clock height Jaka: %d\n", _clockheight);  // Obszar 
  ...
  // Pobranie głównego fontu (może być duży w wygaszaczu)
  const GFXfont* mainFont = clockfont_get(config.store.clockFontId);
  if (config.isScreensaver) {
    gfx.setTextSize(mainFont ? 1 : TIME_SIZE * 4);
  } else {
    gfx.setTextSize(mainFont ? 1 : TIME_SIZE);
  }
  gfx.setFont(mainFont);

  // POPRAWKA: Jeśli font się zmienił (np. po przełączeniu na duży w wygaszaczu),
  // przelicz _timeheight na podstawie aktualnego fonta.
  // _textHeight() korzysta z clockfont_get() więc zwróci właściwą wartość.
  {
    uint16_t newH = _textHeight();
    if (newH != _timeheight) {
      _timeheight = newH;
      
      // _clockheight też przelicz
      #ifndef HIDE_DATE
      if (_fullclock) {
        _clockheight = _timeheight + _space + CHARHEIGHT * _dateheight;
      } else {
  ...
  if (mainFont != s_lastPrintFont) {
    _getTimeBounds();
    s_lastPrintFont = mainFont;
  }
...
if (_fullclock) {
  ...
  if (fullClockOnScreensaver) {
    ...
    if (config.store.hours12) {
      #if TIME_SIZE==19
      const GFXfont* ampmFont = yoScrollFont(0);
      #elif TIME_SIZE==35
      const GFXfont* ampmFont = yoScrollFont(0);
      #elif TIME_SIZE==52
      const GFXfont* ampmFont = yoScrollFont(5);
      #elif TIME_SIZE==70
      const GFXfont* ampmFont = yoScrollFont(1);
      #endif
      
      uint8_t ampmH   = yoFontHeight(ampmFont);

      #if TIME_SIZE==19
      int16_t ampmY = _top()-11;
      #elif TIME_SIZE==35
      int16_t ampmY = _top()-21;
      #elif TIME_SIZE==52
      int16_t ampmY = _top()-40;
      #elif TIME_SIZE==70
      int16_t ampmY = _top()-55;
      #endif
...
void ClockWidget::draw(bool force){
  ...
  const GFXfont* currentFont = clockfont_get(config.store.clockFontId);
  static const GFXfont* s_lastFontPtr = nullptr;
  if (currentFont != s_lastFontPtr) {
    _getTimeBounds(); 
    s_lastFontPtr = currentFont;
    _printClock(true);
  } else {
...
void DateWidget::update() {
  ...
  #ifndef DSP_LCD
  if (_gfxFont)
    desired = yoStringWidth(_gfxFont, dateBuf) + yoGlyphWidth(_gfxFont, ' ') * 2;  //  4
  else {
  ...
  uint16_t minW = (_gfxFont ? yoGlyphWidth(_gfxFont, 'W') : (uint8_t)CHARWIDTH) * 6; // 8
